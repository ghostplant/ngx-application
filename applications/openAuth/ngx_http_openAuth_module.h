...
#define ngx_get_session(r)  (((http_openAuth_ctx_t*)ngx_http_get_module_ctx(r, ngx_http_openAuth_module))->ptr)
#define ngx_set_session(r, data)  (((http_openAuth_ctx_t*)ngx_http_get_module_ctx(r, ngx_http_openAuth_module))->ptr = (data))
...
ngx_module_t ngx_http_openAuth_module;
...
static ngx_int_t ngx_websocket_do_raw_send(ngx_http_request_t *r, u_char *message, size_t len, u_char opcode) {
	http_openAuth_ctx_t *ctx = ngx_http_get_module_ctx(r, ngx_http_openAuth_module);
	if (!ctx->outbuf) {
...
static ngx_int_t ngx_http_openAuth_handler(ngx_http_request_t *r) {
	http_openAuth_ctx_t *ctx = ngx_pcalloc(r->pool, sizeof(http_openAuth_ctx_t));
	if (ctx == NULL)
	...
	ctx->outsz = 0;
	ctx->ptr = NULL;
	ngx_http_set_ctx(r, ctx, ngx_http_openAuth_module);
	...
	if (ws_key.len == 0)
		return ngx_http_openAuth_normal_handler(r);
...
static char *ngx_http_openAuth(ngx_conf_t *cf, ngx_command_t *cmd, void *conf) {
	ngx_http_core_loc_conf_t *clcf = ngx_http_conf_get_module_loc_conf(cf, ngx_http_core_module);
	clcf->handler = ngx_http_openAuth_handler;
	return NGX_CONF_OK;
}

static ngx_command_t ngx_http_openAuth_commands[] = {
	{
		ngx_string("http_openAuth"), /* directive */
		NGX_HTTP_LOC_CONF|NGX_CONF_NOARGS, /* location context and takes no arguments*/
		ngx_http_openAuth, /* configuration setup function */
...
static ngx_http_module_t ngx_http_openAuth_module_ctx = {
...
ngx_module_t ngx_http_openAuth_module = {
	NGX_MODULE_V1,
	&ngx_http_openAuth_module_ctx, /* module context */
	ngx_http_openAuth_commands, /* module directives */
